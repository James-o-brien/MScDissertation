...
ExtractAllCOPERNICUS<-function(dir,haz="TC"){
  ...
  cfiles<-cfiles[grep(haz,cfiles)] 
  cfiles<-cfiles[grep(".shp",cfiles)]  
  cfiles<-cfiles[grep(".shp.xml",cfiles,invert = T)]
...
ExtractAllUNOSAT<-function(dir,haz="TC"){
  ...
  ufiles<-ufiles[grep(haz,ufiles)]
  ufiles<-ufiles[grep(".shp",ufiles)] 
  ufiles<-ufiles[grep(".shp.xml",ufiles,invert = T)]
  ufiles<-ufiles[endsWith(ufiles, ".shp")]
  
  tmp<-strsplit(ufiles,"/",fixed = T) 
  evname<-c()
  for (i in 1:length(tmp)) {
...
ExtractBDfiles<-function(dir="./",haz="TC",org="all",rename=T){
