...
bbox = c(-180, -20.66485, -178.7463, -14.9319)
...
ExtractOSMbuild <- function(bbox, timeout=60, earlyret=F){
  ...
  obj <- opq(bbox = c(bbox), timeout = timeout) %>% add_osm_feature("building") %>% # opq builds an overpass query, which is read-only API that serves up custom selected parts of the OSM map data
  ...
  obj <- dplyr::select(obj$osm_polygons, building.levels, geometry, amenity, name) %>% as("Spatial")
...
GetOSMbuildings<-function(bbox, BD=NULL, minnum=50, plotty=F, timeout=60){ 
  ...
  if(area>0.01){
    tbuildings<-tryCatch(ExtractOSMbuild(bbox = Damage_bbox_1,timeout=timeout),error=function(e) NULL)
...
GetOSMbuildingsODD<-function(ODD,bbox=NULL,minnum=50,plotty=F,timeout=60){
  ...
  if(area>0.01){
    tbuildings<-tryCatch(ExtractOSMbuild(bbox,timeout=timeout),error=function(e) NULL)
    if(is.null(tbuildings)) {
      p<-ggplot(as.data.frame(ODD),aes(bbox[1,],bbox[2,]))+stat_bin_2d(drop = F,binwidth = 0.1)
      pg<-(ggplot_build(p))$data[[1]]; rm(p)
      buildings<-data.frame()
      for(i in 1:nrow(pg)){
        bbox<-as.double(pg[i,c("xmin","ymin","xmax","ymax")])
