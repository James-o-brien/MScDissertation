# ------------------------------------------------------------------------------ #
# Changing GetDisplacements_test to read in excel spreadsheet instead of .Rdata file

#install.packages("xlsx")
library(xlsx)
workflow_test_data <- read.xlsx(file = paste0(dir,"Data_misc/DispData_EQ_V2_workflow_test.xlsx"))
...
GetDisplacements<-function(haz, saved=F, reduce=T, GIDD=F, EMDAT=T, dir="./"){
  
  if(saved) return(ExtractCheckedDisp(dir))
  
  #CM<-GetHelix(haz=haz,reduce=reduce)
  
  Dispy <- read.csv(paste0(dir,"IIDIPUS_Input/DispData_",haz,".csv"))
  CM <- Dispy %>% transmute(
    iso3 = iso3, 
    gmax = gmax, 
    hazard = 'EQ',
