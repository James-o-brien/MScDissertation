...
setMethod(f="initialize", signature="ODDSim",
  ...
  #                        qualifierBD = ifelse(all(is.na(buildDestroyed), NA_character_, qualifierMort[which.max(mortality)])))
            ...
            #.Object@data$nBuildings <- round(runif(1,0.2,1) * total_pop@data + rnorm(length(total_pop@data),0,20))
...
lhazSDF <- simulateODDSim(DamageData)
...
simulateEvent <- function(r, I0 = 3){ 
  ...
  hazsdf<-hazsdf[hazsdf$mmi_mean>4.5,] ############################################### changed this to 4.5 from I0
  colnames(hazsdf@coords)<-rownames(hazsdf@bbox)<-c("Longitude","Latitude")

  return(hazsdf)
}
x <- simulatePopulation(r)
x@data
...
#DamageData is my version of miniDam
simulateODDSim(DamageData)
DamageData$gmax <- rep("", length(DamageData$iso3))
?raster
miniDam <- DamageData

r <- raster(ncol=100, nrow=100, xmn=Damage_bbox_1[1], xmx= Damage_bbox_1[3], ymn=Damage_bbox_1[2], ymx=Damage_bbox_1[4], 
              crs="+proj=longlat +datum=WGS84") #each cell is 30 arcseconds x 30 arcseconds


simulateODDSim(DamageData)
ODDy_new <- simulateODDSim(miniDam)
simulateODDSim <- function(miniDam, I0=4.5){ #changed I0 from 4.5 to 3 
  ...
  lhazdat<-list(hazard_info=list(bbox=bbox,sdate=min(miniDam$sdate),fdate=max(miniDam$fdate),
                                 NumEvents=lenny,hazard="EQ", I0=3, eventdates=rep(miniDam$sdate, lenny)))
...
lhazdat_new$Population
lhazdat

ODDSim <- new('ODDSim', lhazSDF = lhazdat, DamageData = DamageData, PopSim = total_pop, GDPSim = total_GDP)
