...
setMethod("AddHazSDF", "ODD", function(ODD,lhazSDF){
  ...
  for (i in start:lenny){
    ...
    if(lhazSDF$hazard_info$hazard=="TC"){
      ?akima::interp
      hsdf$mean
      hsdf_df <- as.data.frame(hsdf)
      hsdf_df
      layer<-with(as.data.frame(hsdf),akima::interp(x = hsdf_df$Longitude, y = hsdf_df$Latitude, z = hsdf_df$mean,
...
ODD <- readRDS(paste0(dir,"IIDIPUS_Input/ODDobjects_v4/TC20200404VUT_2208_agg_4"))
center <- Model$center
...
setMethod("DispX", "ODD", function(ODD,Omega,center, BD_params, LL=T,
                                   Method=list(Np=2,cores=3,cap=-300), epsilon=c(0.15,0.03,0.1)
){
  ...
  LP<-rep(1,10)
    #GetLP(ODD,Omega,Params,Sinc,notnans)
  ...
  CalcDam<-function(ij){
    ...
    for (h in hrange){
      ...
      bDamage2 <- 0
      bDamage2<-bDamage2+fDamageBuilding(ODD@data[ij,],I_ij,
                                       Params[c("I0","Np","center")],
                                       Omega,
                                       locallinp,
                                       Params[[iso3c]]$Ik)
    }
    bDamage2[bDamage2>=1]<-1-1e-7
    bDamage2[bDamage2<=1e-7]<-1e-7
    
    fnBUnaf <- function(D_BD){
    NB <- rep(0, length(D_BD))
    for (i in 1:length(D_BD)){
      if(D_BD[i] < 0.1){
      NB[i] <- 1  
      }
    }
    ...
    fnBDam <- function(D_BD){
      NB <- rep(0, length(D_BD))
      for (i in 1:length(D_BD)){
        if(D_BD[i] >= 0.1 & D_BD[i] <= 0.5){
          NB[i] <- 1  
        }
      }
      return(NB)
    ...
    D_BD_Unaf <- bDamage2[bDamage2 < 0.1]
    D_BD_Dam <- bDamage2[bDamage2 <= 0.5 & bDamage2 >= 0.1]
    D_BD_Des <- bDamage2[bDamage2 > 0.5]
    
    moreBUnaf = fnBUnaf(D_BD_nBUnaf)
    nBUnaf = nBUnaf + moreBUnaf
    
    moreBDam = fnBDam(D_BD_nBDam)
    nBDam = nBDam + moreBDam
    
    moreBDes = fnBDes(D_BD_nBDes)
...
MakeODDPlots<-function(ODDy, input=NULL){
    ...
    p2<-q+ geom_raster(data = as.data.frame(ODDy),
                               mapping = aes(Longitude,Latitude,fill=hazMean23),
