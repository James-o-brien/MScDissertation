...
DispData<-readRDS(paste0(dir,"IIDIPUS_Input/DispData_",haz,"_V2.Rdata"))
GetDisaster(DispData)
typeof(DispData)
DispData

miniDam

tDisp <- miniDam %>% filter(hazard == "EQ")
EQparams <- ExtractParams("EQ")
bbox <- countriesbbox(unique(tDisp$iso3))
GetUSGS(bbox=bbox, sdate=min(tDisp$sdate), fdate = max(tDisp$sdate), I0=EQparams$I0, minmag=EQparams$minmag)
