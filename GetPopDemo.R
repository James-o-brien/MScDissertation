...
###############################################################################################
info <- readLines(filer[2], n = 6)
filer[1]


SortDemoData(filer = filer, bbox = bbox)
SortDemoData<-function(filer,bbox=NULL){
  
  # print(filer)
  
  info<-readLines(filer[1],n = 6); info<-strsplit(info, "\\s+")
  for (i in 1:6){assign(info[[i]][1],as.numeric(info[[i]][2]))} ; rm(info);
  popdemo<-read.csv(filer[1],header = FALSE,skip = 6,sep = " ",na.strings = NODATA_value,colClasses = "numeric")
...
GetPopulationBbox<-function(directory,bbox,density=F,lowres=FALSE,yr="2020",plotty=FALSE, namer="Population",ncity=1){
  # bbox is bounding box in the form 'min lon, max lat, max lon, min lat'
  # DATA: NASA - SEDAC
  
  if(density){ctds<-"density"} else {ctds<-"count"}
  
  if(abs(bbox[2])>90 | abs(bbox[4])>90 | abs(bbox[1])>180.1 | abs(bbox[3])>180.1) {stop("Error: non-physical bounding box values in GetPopulationBbox")}
